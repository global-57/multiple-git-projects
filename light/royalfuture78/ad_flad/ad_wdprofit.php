...
function confirmation(noid) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=wdprofit&page=delete&no=" + noid;
...
<h2><img src="images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Withdrawal Profit</h2>
<p align="center">&nbsp;</p>
...
$jml_byridr = $row35['idr'];
	...
	if($tjn == 2){
//$db->update("datacwalet", "status='1'", "kode='$kode' and tujuan='$username'");	
}	
...
if($hp){ 
$isipesan = "Selamat ".$nama.", withdrawal profit anda senilai ".$jumlahdeponec." telah diproses.";
...
<p>Withdraw profit anda telah diproses.</p>
...
$mail3->Password   = $smtpass;        // SMTP account password
	    ...
	    $mail3->IsHTML(true);       
        $mail3->Subject = ''.$nama.', Proses withdrawal profit';
        $mail3->msgHTML($isimail);
	  //  $mail3->AddAttachment("../invoice/".$invc.".pdf");      // attachment
        $mail3->send();	


header("location: ?go=wdprofit");
		exit;
?>
<?php		
...
if($tjn == 2){
//$db->update("dataewalet", "status='0'", "kode='$kode' and tujuan='$username'");	
}
header("location: ?go=wdprofit");
		exit;
?>
...
if(isset($_GET["keywrd"])){ 
	$where = "jenis='2' and userid = '".$_GET["keywrd"]."'";
} else {
	$where = "jenis='2'";
}	
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <input id="go" name="go"  type="hidden" value="wdprofit" />
...
<button type='submit' class="submitkecil">LIHAT TANGGAL</button>
 <a href="?go=wdprofit"><button type="button" class="submitkecil"/>LIHAT SEMUA</button></a>
</td>
        </tr>
      </table>
    </form>
...
if($db->result($i, "status") > 0) {
		$img = "<a href='?go=wdprofit&page=publish&pub=0&no=".$no."&mid=".$username."&kode=".$kode."'><button class='primadetail' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk batalkan proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx2cex()'>Done</button></a>";
	} else {
		$img = "<a href='?go=wdprofit&page=publish&pub=1&no=".$no."&mid=".$username."&kode=".$kode."'><button class='primaback' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx2ce()'>Pending</button></a>";
	}
...
?>
  ...
  <a href="?go=wdprofit<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?go=wdprofit<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?>&show=<?= $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
  <?php
...
?>
[ <a href="?go=wdprofit<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?>&show=<?= $i; ?>" style="font-size:10px; color:#0000CC">
<?= $i; ?>
</a> ]
<?php
...
?>
| <a href="?go=wdprofit<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?>&show=<?= $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?go=wdprofit<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?>&show=<?= $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
