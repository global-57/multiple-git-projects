...
function confirmation(noid) {
	var answer = confirm("Are You sure to delete this album?")
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=album&page=delete&no=" + noid;
...
<h2><img src="images/icon-48-menumgr.png" width="48" height="48" align="absmiddle" /> Album Foto Manager</h2>
<div id="menu_button2">
  <ul>
   <li><a href="?go=album&page=addnew&edit=0"><img src="images/add.png" width="12" align="absbottom" />&nbsp;&nbsp;<strong>Tambah Album Foto</strong></a></li>
  </ul>
</div>
<?php
...
if(isset($_GET["no"])){ $no = $_GET["no"]; }
	...
	if($edit == 1) {
		$db->select("id, nama, detail, url, published, banner, hits", "bannerhome", "id=$no");	
...
?>
<form action="?go=album&page=submit" method="post" enctype="multipart/form-data" name="form2" onsubmit="return Validate(this);">
  <table width="80%" border="0" align="center" cellpadding="5" cellspacing="1">
    ...
    <tr>
      <td width="35%" align="right">Nama Album :</td>
      <td width="65%"><label>
    ...
    <tr>
      ...
      <td>
	    <input name="link" type="text" class="form" id="link" value="<?php echo $link; ?>" size="50" />
	  </td>
    </tr>
	 <tr>
...
//You should create the validator only after the definition of the HTML form
  var frmvalidator  = new Validator("form2");
	  frmvalidator.addValidation("link","req","Masukan Link Album");
...
$db->select("ordering", "bannerhome", "", "ordering");
   ...
   $fotone = $_POST['fotone'];
		...
		if($edit > 0) {
				$db->update("bannerhome", "nama='$title', banner='$foto', url='$link', detail='$ket', published='$publish', tgl='$clientdate'", "id='$no'");
				echo "<meta http-equiv='refresh' content='0;URL=?go=album&page=addnew&edit=1&no=$no'>";
	
	
	} else {	
		$db->insert("bannerhome", "nama, banner, detail, url, hits, ordering, published, tgl", "'$title', '$foto', '$ket', '$link', 0, '$order', '$publish', '$clientdate'");
	echo "<meta http-equiv='refresh' content='0;URL=?go=album'>";
			}
	}
	}
?>
...
if(isset($_GET["kat"])){ $kat = $_GET["kat"]; }	
		...
		$db->update("bannerhome", "ordering='$new_ord'", "id='$no'");
		$db->update("bannerhome", "ordering='$new_ord1'", "id='$no_id'");
		echo "<meta http-equiv='refresh' content='0;URL=?go=album'>";
?>
<?php
} else {
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbl_header">
    ...
    <td width="15%" align="center">Nama Album </td>
	<td width="23%" align="center">Link</td>
    <td width="14%" align="center">Cover</td>
    <td width="27%" align="center">Info</td>
...
$start = (($display * $limit) - $limit);
	$numrows = $db->count_records("bannerhome", "");
$db->select("id, nama, detail, url, banner, hits, published, ordering", "bannerhome", "", "ordering", "", "", "$start, $limit");
...
for($i=0;$i<$j;$i++) {
	...
	if($db->result($i, "published") > 0) {
		$img = "<a href='?go=album&page=publish&pub=0&no=".$db->result($i, "id")."'><img src='images/tick.png' border=0 title='Click to Unpublish'></a>";
	} else {
		$img = "<a href='?go=album&page=publish&pub=1&no=".$db->result($i, "id")."'><img src='images/publish_x.png' border=0 title='Click to Publish'></a>";
	} 	
	//---ordering---
	if($db->result($i, "ordering") == 1) {
		$ordering = "<a href='?go=album&page=ordering&step=1&ord=".$db->result($i, "ordering")."&nextid=".$db->result($nom, "id")."&no=".$db->result($i, "id")."'><img src='images/downarrow.png' border=0 title='Move Down'></a>";
	} else if($db->result($i, "ordering") > 1 and $db->result($i, "ordering") < $j) {
		$ordering = "<a href='?go=album&page=ordering&step=-1&ord=".$db->result($i, "ordering")."&lastid=".$db->result($lid, "id")."&no=".$db->result($i, "id")."'><img src='images/uparrow.png' border=0 title='Move Up'></a>&nbsp;&nbsp;<a href='?go=album&page=ordering&step=1&ord=".$db->result($i, "ordering")."&nextid=".$db->result($nom, "id")."&no=".$db->result($i, "id")."'><img src='images/downarrow.png' border=0 title='Move Down'></a>";	
	} else if($db->result($i, "ordering") == $j) {	
		$ordering = "<a href='?go=album&page=ordering&step=-1&ord=".$db->result($i, "ordering")."&lastid=".$db->result($lid, "id")."&no=".$db->result($i, "id")."'><img src='images/uparrow.png' border=0 title='Move Up'></a>";
	}	
	...
	$dirfoto = "../gallery/foto/$adafoto";
	if (!empty($adafoto) && (file_exists($dirfoto))){
		$gambar = "<a href='../gallery/foto/".$adafoto."' class='highslide' onclick='return hs.expand(this)'><img src='../gallery/foto/$adafoto' class='imgFloatLeft' height='75' width='75'></a>";
		}
	else
		{
		$gambar = "<a href='../images/photo_not_available.jpg' class='highslide' onclick='return hs.expand(this)'><img src='../images/photo_not_available.jpg' class='imgFloatLeft' height='75' width='75'></a>";
		} 		 
?>
  ...
  <tr class="<?php echo $class; ?>">
	...
	<td align="center"><?php echo $img; ?></td>
	 ...
	 <td align="center"><a href="?go=album&page=addnew&edit=1&no=<?php echo $db->result($i, "id"); ?>"><img src='images/edit_f2.png' border=0 title='Click to Edit'></a></td>
    <td align="center"><a href="#" onClick='confirmation(<?php echo $db->result($i, "id"); ?>)' style='cursor:hand'><img src='images/stop_f2.png' border=0 title='Click to Hapus'></a></td>
  </tr>
<?php
	}
?>	  
...
if ($display > 1) {
	$previous = $display - 1;
?>
  <a href="?go=album&kat=<?php echo $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?go=album&kat=<?php echo $kat; ?>&show=<?php echo $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
<?php
}
if ($numrows != $limit) {
	if ($scroll == 1) {
...
?>
[ <a href="?go=album&kat=<?php echo $kat; ?>&show=<?php echo $i; ?>" style="font-size:10px; color:#0000CC">
<?php echo $i; ?>
</a> ]
<?php
		}
...
if ($display < $paging) {
	$next = $display + 1;
?>
| <a href="?go=album&kat=<?php echo $kat; ?>&show=<?php echo $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?go=album&kat=<?php echo $kat; ?>&show=<?php echo $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
}
?>
    </td>
