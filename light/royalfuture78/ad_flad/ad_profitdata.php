...
<input name="id" type="hidden" id="id" value="<?php echo $id; ?>" size="5">
 ...
 for($ix=1;$ix<=365;$ix=$ix+1){
  ...
  ?>
       ...
       <tr> 
      <td align="right" bgcolor="#FFFFFF">Cek Profit:      </td>
      <td bgcolor="#FFFFFF"><input type="radio" name="cekprofit" value="1" id="RadioGroupa1ds_0" checked="checked"/>
          cek bila sudah dibagikan
          <input type="radio" name="cekprofit" value="0" id="RadioGroupa1ds_1"/>
        tidak
...
if($numo && $cekprofit==1) {
	...
	$komspontocash = ($pembagianwdne1/100)*$jumlah;	
			$komspontoregs = ($pembagianwdne2/100)*$jumlah;	
			//$komspontokom = $komsponx-$komspontostore;	
			
            $komspontocash=sprintf("%.2f",$komspontocash);	
            $komspontoregs=sprintf("%.2f",$komspontoregs);		
...
//$db->insert("datacwalet", "", "'', '$kode', 'administrator', '$komspontocash', '".$pembagianwdne1."% ".$jenisejam."', '$usernamenya', '$tanggalkom', 1, '$tanggalkom', '', ''");
//$db->insert("dataawalet", "", "'', '$kode', 'administrator', '$komspontoregs', '".$pembagianwdne2."% ".$jenisejam."', '$usernamenya', '$tanggalkom', 1, '$tanggalkom', '', ''");
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <input id="go" name="go"  type="hidden" value="profit-data" />
...
<button type='submit' class="submitkecil">Lihat</button>
 <a href="?go=profit-data"><button type="button" class="submitkecil"/>Lihat Semua</button></a>
</td>
        </tr>
      </table>
    </form>
...
if($db->result($i, "gett") == 1) {
		$getbonus = "<a href='?go=profit-data&page=lock&lock=0&no=".$no."&mid=".$user1."&kode=".$kode."'><button class='primaback' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Unlock This Bonus')\" onMouseout='hideddrivetip()'>Locked</button></a>";

		}else{
		$getbonus = "<a href='?go=profit-data&page=lock&lock=1&no=".$no."&mid=".$user1."&kode=".$kode."'><button class='mmm_blue' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Lock This Bonus')\" onMouseout='hideddrivetip()'>Active</button></a>";
		}		
?>
...
<tr  class="<?php echo $class; ?>" > 
    <td align="center"><?php echo $nom; ?></td>
                           ...
                           <td align="center"><?php echo $style; ?><a href="?go=profit-data&page=addnew&edit=1&id=<?php echo $no; ?>"><img src='../images/edit_f2.png' border=0 title='Edit Data' width="24"></a></font></td>
                             <td align="center"><?php echo $style; ?><a href="?go=profit-data&page=delete&id=<?php echo $no; ?>"><img src='../images/errormsg.png' border=0 title='Delete Data' width="24"></a></font></td>
  </tr>
...
?>
  <span class='prevnext'><a href="?go=profit-data<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?><?= $frome; ?><?= $toe; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?go=profit-data&show=<?php echo $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a></span> |
  <?php
...
?>
<a href="?go=profit-data<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?><?= $frome; ?><?= $toe; ?>&show=<?php echo $i; ?>" style="font-size:10px; color:#0000CC">
<?php echo $i; ?>
</a>
<?php
...
?>
| <span class='prevnext'><a href="?go=profit-data<?= $blndt; ?><?= $thndt; ?><?= $kwde; ?><?= $frome; ?><?= $toe; ?>&show=<?php echo $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a></span> | <span class='prevnext'><a href="?go=profit-data&show=<?php echo $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a></span>
<?php
