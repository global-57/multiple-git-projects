...
function confirmation(noid) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=wdrwx&page=delete&no=" + noid;
...
<h2><img src="images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Withdrawal Reward Sponsor</h2>
<p align="center">&nbsp;</p>
...
$sql = mysql_query("select * from wd_reward2 order by id desc LIMIT $posisi,$batas");
$nom = 1;
...
if($row[6] > 0) {
		$img = "<a href='?go=wdrwx&page=publish&pub=0&no=$row[0]&mid=$row[2]&kode=$row[1]'><button class='primapc' style='padding:4px 8px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk batalkan proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx2()'>Proses</button></a>";
	} else {
		$img = "<a href='?go=wdrwx&page=publish&pub=1&no=$row[0]&mid=$row[2]&kode=$row[1]'><button class='primapc2' style='padding:4px 8px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx1()'>Pending</button></a>";
	}
...
$tampil2 = mysql_query("SELECT * FROM wd_reward2");
...
if($halaman > 1){
	$prev=$halaman-1;
	echo "<span class=prevnext><a href=?go=wdrwx&halaman=$prev>Prev</a></span> ";
}
else{ 
...
for($i=1;$i<=$jmlhal;$i++)
if ($i != $halaman){
	echo " <a href=?go=wdrwx&halaman=$i>$i</a> ";
}
else{
...
if($halaman < $jmlhal){
	$next=$halaman+1;
	echo "<span class=prevnext><a href=?go=wdrwx&halaman=$next>Next</a></span>";
}
else{ 
...
if(isset($_GET["kode"])){ $kode = $_GET["kode"]; }
		...
		$db->update("wd_reward2", "status='$pub', tglproses='$clientdate'", "id='$no'");
		
		echo "<meta http-equiv='refresh' content='0;URL=?go=wdrwx'>";
?>
<?php		
...
$db->update("wd_reward2", "status='$pub', tglproses=''", "id='$no'");
