...
function confirmation(mid, page, action) {
	var answer = confirm("Process Transfer E-walet to this Member: " + mid + " ?")
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=proewalet&page=" + page + "&mid=" + mid + "&action=" + action;
	}
}
//-->
</script>
...
<div class="cc01">
<h2><img src="images/coins.png" width="48" height="48" align="absmiddle"> ADD FUND BALANCE</h2>
<?php
if (isset($_GET['page']) && $_GET['page'] == "edit") {
	
if(isset($_GET["user"])){ $user = $_GET["user"]; }
if(isset($_GET["kode"])){ $kode = $_GET["kode"]; }
if(isset($_GET["jn"])){ $jenis = $_GET["jn"]; }
	//---tgl mbr pertama aktif--
if($jenis == "free"){
		$databasenya="dataswalet";
			$jenisnya="Free Wallet Balance";
			}else if($jenis == "cash"){
		$databasenya="datacwalet";
			$jenisnya="Cash Wallet Balance";
			}
			
$query113 = "SELECT * FROM $databasenya WHERE kode='$kode'"; 
...
$accid2 = $row113['accid2'];
	...
	?>
<form id="ewalet" name="form1" method="post" action="?go=emoney&page=editgo">
  ...
  <table width="80%" border="0" align="center" cellpadding="5" cellspacing="4" class="table">
   <tr class="tbl_header">
    ...
    <tr>
      ...
      <td width="50%"><label>
       <input name="" type="text" id="" size="20" disabled="disabled"  value="<?php echo $tujuan; ?>"/>
       ...
       <input name="jn" type="hidden" id="jn" size="20" value="<?php echo $jenis; ?>"/>
    ...
    <tr>
      <td width="50%" align="right">Nilai (USD):</td>
      <td width="50%"><label>
...
$db->update("$databasenya", "jumlah='$jumlah'", "kode='$kodene'");
...
<br /><br />
 <script language="Javascript">
        ...
        function redirectToFB(){
            window.opener.location.href="index.php?go=ewalet&page=<?php echo $jenisee; ?>&user=<?php echo $tujuan; ?>";
            self.close();
        }
...
<?php
	...
	$tujuan = $_POST['tujuan'];
		...
		$databasenya="datacwalet";
			...
			$db->select("kode", "$databasenya", "kode='$kode'");
			$ada = $db->num_rows();
if ($ada >0) {
    echo "<br><br><div class='errorx'>Transaksi ini sudah diproses sebelumnya</div><br><br>";
} else {
			
$db->insert("$databasenya", "", "'', '$kode', 'administrator', '$jumlah', '$ket', '$tujuan', '$clientdate', 1, '$clientdate', 'administrator', '$accid'"); 	

}	


if($typene == 1){
$db->update("member", "free='0'", "username='$tujuan'");	
}
...
<table width="80%" border="0" align="center" cellpadding="5" cellspacing="5">
      <tr class="tbl_header">
        <td colspan="2" align="center" style="color:#FF0000"><h3>ADD BALANCE E-MONEY SUCCESS!</h3></td>
      </tr>
      <tr>
       ...
       <tr>
        <td width="50%" align="right">Wallet Balance : </td>
        <td width="50%"><strong>
        <?php echo $jenisnya; ?>
        </strong></td>
      </tr>
      <tr>
        <td width="50%" align="right">User ID tujuan : </td>
        <td width="50%"><strong>
        <?php echo $tujuan; ?>
        </strong></td>
      </tr>
...
<br /><br />
 <script language="Javascript">
        ...
        function redirectToFB(){
            window.opener.location.href="index.php?go=ewalet&user=<?php echo $tujuan; ?>";
            self.close();
        }
...
$kode = substr(str_shuffle(str_repeat("4453B141119A06676420371LPMBTEFWX112D8717497783C6255363423ABCYWTGEHDLPMBTEFWXVU96411241472162223777", 64)), 0, 13);
 ...
 ?>
<form id="ewalet" name="form1" method="post" action="?go=emoney&page=kirim">
 <div class="form_style">
                        <fieldset>
  <table width="80%" border="0" align="center" cellpadding="5" cellspacing="4">
    ...
    <tr>
      ...
      <td width="50%"><label>
        <select name="tujuan" id="tujuan" required='required'>
      
      <option value="000" selected="selected">[ Pilih User ID ]</option>
      
      <?php
	 	$db->select("accid, username", "member", "", "accid");
		...
		echo "<option value='$row[1]'$sell>$row[1] - $row[0]</option>";
		}
		?>	
        </select>
      </label></td>
     ...
     <tr>
      <td width="50%" align="right">Jenis :</td>
      <td width="50%"><label>
       <select name="jenis" id="jenis" required='required'>
              <option value="free">Free Wallet</option>
              <option value="cash" selected="selected">Cash Wallet</option>
		  </select> 
      </label></td>
    </tr>
   
    <tr>
      <td align="right">Nilai (USD):</td>
