...
$sql = "SELECT domain, bisnisname, alamat, email, invimage, hpsms, privatekeydash, publickeydash, kurs, kursidr FROM configuration";
...
if ($result->num_rows > 0) {
	...
	$hpadmin=$row["hpsms"];
    $publickeysdash=$row["publickeydash"];
    $privatekeysdash=$row["privatekeydash"];
	$kurs=$row["kurs"];
	$kursidr=$row["kursidr"];
...
{
$new_amount= number_format($amount,8)." DASH";
  return $new_amount;
}
function point($amount) {
 ...
 $cryptobox_private_keys = array($privatekeysdash);
