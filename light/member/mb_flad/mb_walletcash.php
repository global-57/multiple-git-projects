...
<div class="d-flex justify-content-between align-items-center" >
	<div class="">
		<h5 class="mb-0" style="color:#666666;"> Balance </h5>
	</div>
...
<p class="mb-0" style="color:#666666;"> Your Available Free Balance<br /> 
<font style="font-size:24px; color:#730103;"><?  $saldoawalete = $db->myswalet($user_session);
...
<?php } else { ?>
<p class="mb-0" style="color:#666666;"> Your Available Real Balance<br /> 
<font style="font-size:24px; color:#057E2C;"><?  $saldoawalete = $db->mycwalet($user_session);
	...
	$db->select("kode, uraian, username, jumlah, tujuan, tgl, status, tglproses, accid, accid2", "dataswalet", "username='$user_session' or tujuan='$user_session'", "tgl desc");
		...
		if($row[4] == $user_session) {
			...
			if($row[2] == $user_session) {
				$type = "<font color='#A4080A'>Debet</font>";
			} else {
				$type = "<font color='#057E2C'>Credit</font>";
			}	
			if(is_odd($nom) == 0) {
				$class = "tblrow_ganjil";
			} else {
		...
		$dtpros = "<font color='#057E2C'><i class='fa fa-check'></i>&nbsp;Done</font>";
	}else{
		$dtpros = "<font color='#057E2C'><i class='fa fa-check'></i>&nbsp;Done</font>";
	}
			if($row[6] > 0) {
				...
				$st = "<font color='#8A4A06'><i class='fa fa-spinner fa-spin'></i>&nbsp;Waiting</font>";
			}	
			
			if($row[2] == $user_session) {
				$style = "<font color='#8A4A06'>";
			} else {
				$style = "<font>";
			}	
