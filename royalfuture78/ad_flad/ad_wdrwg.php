...
function confirmation(noid) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=wdrwg&page=delete&no=" + noid;
...
<h2><img src="images/icon-48-user.png" width="48" height="48" align="absmiddle" /> Withdrawal Reward Group</h2>
<p align="center">&nbsp;</p>
...
if($row[6] > 0) {
		$img = "<a href='?go=wdrwg&page=publish&pub=0&no=$row[0]&mid=$row[2]&kode=$row[1]'><button class='primapc' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk batalkan proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx2()'>Proses</button></a>";
	} else {
		$img = "<a href='?go=wdrwg&page=publish&pub=1&no=$row[0]&mid=$row[2]&kode=$row[1]'><button class='primapc2' style='padding:0px 7px;font-size:11px;' onMouseover=\"ddrivetip('Klik disini untuk proses')\" onMouseout='hideddrivetip()' onclick='return confirmActionx1()'>Pending</button></a>";
	}
...
if($halaman > 1){
	$prev=$halaman-1;
	echo "<span class=prevnext><a href=?go=wdrwg&halaman=$prev>Prev</a></span> ";
}
else{ 
...
for($i=1;$i<=$jmlhal;$i++)
if ($i != $halaman){
	echo " <a href=?go=wdrwg&halaman=$i>$i</a> ";
}
else{
...
if($halaman < $jmlhal){
	$next=$halaman+1;
	echo "<span class=prevnext><a href=?go=wdrwg&halaman=$next>Next</a></span>";
}
else{ 
...
if(isset($_GET["kode"])){ $kode = $_GET["kode"]; }
		...
		echo "<meta http-equiv='refresh' content='0;URL=?go=wdrwg'>";
?>
<?php		
