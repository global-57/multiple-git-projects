   ...
   echo "<div class='alert-box errors'><span>Error : </span>Sorry, only JPG, JPEG, PNG & GIF files are allowed to upload.&nbsp;&nbsp;<button onclick='history.go(-1);'>Go back</button></div>";
	...
	if(!empty($_FILES['uploadfile']['name']) && $img['size'] > 4000000) {
		...
		if($edit > 0) {
				...
				$db->update("banner_rek", "namabank='$namabank', banner='$foto', norek='$norek', pemilik='$pemilik', published='$publish'", "id='$no'");
				
        header("location: ?go=bannerrek");
	exit;		
	
	} else {	
		$db->insert("banner_rek", "nama, banner, url, hits, ordering, published, namabank, norek, pemilik", "'$title', '$foto', '$link', 0, '$order', '$publish', '$namabank', '$norek', '$pemilik'");
