...
function confirmation(noid) {
	...
	if (answer){
		//alert("Bye bye!")
		window.location = "?go=bannerpage&page=delete&no=" + noid;
...
<div id="menu_button2">
  <ul>
   <li><a href="?go=bannerpage&amp;page=addnew&edit=0"><img src="images/add.png" width="12" align="absbottom" />&nbsp;&nbsp;<strong>Tambah Header Page</strong></a></li>
  </ul>
</div>
<?php
...
?>
<form action="?go=bannerpage&page=submit" method="post" enctype="multipart/form-data" name="form2"  onsubmit="return Validate(this);">
  <table width="80%" border="0" align="center" cellpadding="5" cellspacing="1">
    ...
    <tr>
      <td width="35%" align="right">Title H1 :</td>
      <td width="65%"><label>
 ...
 <tr>
      <td align="right">Title H2 :</td>
      <td><input name="teks" type="text" id="teks" value="<?php echo $teks; ?>" size="50" maxlength="100"></td>
    </tr>
    <tr>
      <td align="right">class :</td>
      <td><input name="link" type="text" id="link" value="<?php echo $menutype; ?>" size="50" maxlength="100"></td>
    </tr>
    <tr>
    ...
    <tr>
      <td>&nbsp;</td>
      <td><label>
        <input type="submit" name="button2" id="button2" class="submit" value="Simpan Gambar" >
      </label></td>
    </tr><tr>
      <td>&nbsp;</td>
      <td></td>
    ...
    for (var i = 0; i < arrInputs.length; i++) {
        ...
        if (oInput.type == "file") {
            ...
            if (sFileName.length > 0) {
                ...
                if (!blnValid) {
                    alert("Maaf, " + sFileName + " tidak di ijinkan di upload untuk header page, silahkan upload hanya file image : " + _validFileExtensions.join(", "));
...
$db->select("ordering", "banner_kiri", "", "ordering");
		$j=$db->num_rows();
		$order = $j + 1;
   $fotone = $_POST['fotone'];
		if(!empty($thumbName)){ 
	...
	if($edit > 0) {
		$db->update("banner_kiri", "nama='$title', banner='$foto', url='$link', published='$publish', teks='$teks'", "id='$no'");
		echo "<meta http-equiv='refresh' content='0;URL=?go=bannerpage'>";
	} else {	
		$db->insert("banner_kiri", "nama, banner, url, hits, ordering, published, teks", "'$title', '$foto', '$link', 0, '$order', '$publish', '$teks'");
		echo "<meta http-equiv='refresh' content='0;URL=?go=bannerpage'>";
	}
}
}
...
if(isset($_GET["pub"])){ $pub = $_GET["pub"]; }
		$db->update("banner_kiri", "published='$pub'", "id='$no'");
		echo "<meta http-equiv='refresh' content='0;URL=?go=bannerpage'>";
?>
<?php
} else if (isset($_GET['page']) && $_GET['page'] == "delete") {
if(isset($_GET["no"])){ $no = $_GET["no"]; }
		$sqlber = mysql_query("SELECT * FROM banner_kiri WHERE id='$no'");
...
if(isset($_GET["kat"])){ $kat = $_GET["kat"]; }	
		...
		$db->update("banner_kiri", "ordering='$new_ord'", "id='$no'");
		$db->update("banner_kiri", "ordering='$new_ord1'", "id='$no_id'");
		echo "<meta http-equiv='refresh' content='0;URL=?go=bannerpage'>";
?>
<?php
} else {
...
$start = (($display * $limit) - $limit);
	$numrows = $db->count_records("banner_kiri", "");

$db->select("id, nama, url, banner, hits, published, ordering", "banner_kiri", "", "ordering", "", "", "$start, $limit");
...
for($i=0;$i<$j;$i++) {
	...
	if($db->result($i, "published") > 0) {
		$img = "<a href='?go=bannerpage&page=publish&pub=0&no=".$db->result($i, "id")."'><img src='images/tick.png' border=0 title='Click to Unpublish'></a>";
	} else {
		$img = "<a href='?go=bannerpage&page=publish&pub=1&no=".$db->result($i, "id")."'><img src='images/publish_x.png' border=0 title='Click to Publish'></a>";
	} 	
	//---ordering---
	if($db->result($i, "ordering") == 1) {
		$ordering = "<a href='?go=bannerpage&page=ordering&step=1&ord=".$db->result($i, "ordering")."&nextid=".$db->result($nom, "id")."&no=".$db->result($i, "id")."'><img src='images/downarrow.png' border=0 title='Move Down'></a>";
	} else if($db->result($i, "ordering") > 1 and $db->result($i, "ordering") < $j) {
		$ordering = "<a href='?go=bannerpage&page=ordering&step=-1&ord=".$db->result($i, "ordering")."&lastid=".$db->result($lid, "id")."&no=".$db->result($i, "id")."'><img src='images/uparrow.png' border=0 title='Move Up'></a>&nbsp;&nbsp;<a href='?go=bannerpage&page=ordering&step=1&ord=".$db->result($i, "ordering")."&nextid=".$db->result($nom, "id")."&no=".$db->result($i, "id")."'><img src='images/downarrow.png' border=0 title='Move Down'></a>";	
	} else if($db->result($i, "ordering") == $j) {	
		$ordering = "<a href='?go=bannerpage&page=ordering&step=-1&ord=".$db->result($i, "ordering")."&lastid=".$db->result($lid, "id")."&no=".$db->result($i, "id")."'><img src='images/uparrow.png' border=0 title='Move Up'></a>";
	}	
...
?>
  ...
  <tr class="<?php echo $class; ?>">
   ...
   <td align="center"><?php echo $gambar; ?></td>
    <td align="center"><a href="?go=bannerpage&page=addnew&edit=1&no=<?php echo $db->result($i, "id"); ?>"><img src='images/edit_f2.png' border=0 title='Click to Edit'></a></td>
...
if ($display > 1) {
	$previous = $display - 1;
?>
  <a href="?go=bannerpage&kat=<?php echo $kat; ?>&show=1" style="font-size:10px; color:#0000CC"><< Awal </a> | <a href="?go=bannerpage&kat=<?php echo $kat; ?>&show=<?php echo $previous; ?>" style="font-size:10px; color:#0000CC">< Sebelumnya </a> |
<?php
}
if ($numrows != $limit) {
	if ($scroll == 1) {
...
?>
[ <a href="?go=bannerpage&kat=<?php echo $kat; ?>&show=<?php echo $i; ?>" style="font-size:10px; color:#0000CC">
<?php echo $i; ?>
</a> ]
<?php
		}
...
if ($display < $paging) {
	$next = $display + 1;
?>
| <a href="?go=bannerpage&kat=<?php echo $kat; ?>&show=<?php echo $next; ?>" style="font-size:10px; color:#0000CC">Selanjutnya ></a> | <a href="?go=bannerpage&kat=<?php echo $kat; ?>&show=<?php echo $paging; ?>" style="font-size:10px; color:#0000CC">Terakhir >></a>
<?php
}
?>
    </td>
