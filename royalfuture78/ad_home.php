...
$tgl_skr = (date("Y-m-d"));
 ...
 $cektotdeponetdays=mysql_query("select SUM(amount) from lostwin");
	                 while($rowcektotdeponetdays=mysql_fetch_row($cektotdeponetdays)) {
		             $totdeponeetdays = $rowcektotdeponetdays[0];
					 }			 
					 	 $cektotdepones=mysql_query("select SUM(jml) from deposit");
	                 while($rowcektotdepones=mysql_fetch_row($cektotdepones)) {
		             $totdeponees = $rowcektotdepones[0];
					 }		
					  $cektotbonusee=mysql_query("select SUM(bayar) from komisi WHERE jenis<>'komshare'");
	                 while($rowcektotbonusee=mysql_fetch_row($cektotbonusee)) {
		             $totkomisineee = $rowcektotbonusee[0];
					 }		 
					 
			$cektotprofite=mysql_query("select SUM(bayar) from komisi WHERE jenis='komshare' or jenis='komtrade'");
	                 while($rowcektotprofite=mysql_fetch_row($cektotprofite)) {
		             $totprofiteee = $rowcektotprofite[0];
					 }			  
    ...
    <td width="63%" valign="top"><table width="100%" height="311" border="0" cellpadding="2" cellspacing="4">
      ...
      <tr>
        <td align="center"><a href="?go=faq"><img src="images/icon-48-user-profile.png" alt="Card Center" width="48" height="48" /><br />
          FAQ</a></td>
        <td align="center"><a href="?go=konfirmasipayment"><img src="images/icon-48-menumgr.png" width="48" height="48" border="0" /><br />
          Konfirmasi</a> </td>
...
Configuration</a></td>
    ...
    <table width="100%" height="228" border="1" cellpadding="5" cellspacing="0" bordercolor="#CCCCCC" style="border-collapse:collapse">
	 ...
	 <tr>
        <td width="44%" align="right" style="font-weight:bold">Total Trade : </td>
        <td width="56%" style="font-weight:bold">';echo rupiah($totdeponeetday);echo '</td>
      </tr>
	   <tr>
        <td width="44%" align="right" style="font-weight:bold">Total Invest '.$coiname.': </td>
        <td width="56%" style="font-weight:bold">';echo rupiah($totdeponee);;echo '</td>
      </tr>
	   <tr>
        <td width="44%" align="right" style="font-weight:bold">Total Profit : </td>
        <td width="56%" style="font-weight:bold">';echo rupiah($totprofiteee);;echo '</td>
      </tr>
	   <tr>
        <td width="44%" align="right" style="font-weight:bold">Total Bonus : </td>
        <td width="56%" style="font-weight:bold">';echo rupiah($totkomisineee);;echo '</td>
      </tr>
	   <tr>
        <td colspan="2"></td>
      </tr>
