...
$sql = "SELECT domain, bisnisname, alamat, email, invimage, hpsms, privatekeyltc, publickeyltc, kurs, kursidr FROM configuration";
...
if ($result->num_rows > 0) {
	...
	$hpadmin=$row["hpsms"];
    $publickeysltc=$row["publickeyltc"];
    $privatekeysltc=$row["privatekeyltc"];
	$kurs=$row["kurs"];
	$kursidr=$row["kursidr"];
...
function ltc($amount)
{
$new_amount= number_format($amount,8)." LTC";
  return $new_amount;
}
function point($amount) {
 ...
 $cryptobox_private_keys = array($privatekeysltc);
