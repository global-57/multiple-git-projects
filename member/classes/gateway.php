...
class BtcGateway {
	...
	function create_payment_box($amount, $scne) {
		global $gateway;
		...
		$_SESSION['btc_address'] = $gateway['bitcoin_address'];
		$_SESSION['btc_amount'] = $amount;
		$_SESSION['kodeinvest'] = $scne;
		$address = $gateway['bitcoin_address'];
		$box = '<div class="panel panel-default" id="PaymentBox_'.$_SESSION[btc_boxID].'">
			<div class="panel-body">
				<div class="row">
					<div class="col-sm-12 col-md-12 col-lg-12">
						<h3><span class="text text-warning"><img src="assets/imgs/Bitcoin.png" width="32px" height="32px"></span> Bitcoin Payment</h3>
					</div>
	...
	function QRCode() {
		...
		$scne = $_SESSION['kodeinvest']; 
		return '<img src="https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=bitcoin:'.$address.'?amount='.$amount.'&scne='.$scne.'&choe=UTF-8" class="img-responsive">';
	}
...
class LtcGateway {
	...
	function QRCode() {
		...
		return '<img src="https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=litecoin:'.$address.'?amount='.$amount.'&choe=UTF-8" class="img-responsive">';
	}
...
class DogeGateway {
	...
	function QRCode() {
		...
		$address = $_SESSION['ltc_address'];
		$amount = $_SESSION['ltc_amount']; 
		return '<img src="https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=dogecoin:'.$address.'?amount='.$amount.'&choe=UTF-8" class="img-responsive">';
	}
