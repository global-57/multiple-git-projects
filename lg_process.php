...
$url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . urlencode($secret_key) . '&response=' . $captcha;   
   ...
   if ($usecaptcha==1 && !$recaptcha['success']) { 
         header("location: ./login.php?result=wrong_captcha".$useref."");
      } else {
...
$userlogins = anti_injection($_POST['userlogin']); 
$passlogin=md5($_POST['passlogin']);

$cekemailnyauser = "SELECT username FROM member WHERE email='".$userlogins."' or username='".$userlogins."'"; 
$resultemailnyauser = mysql_query($cekemailnyauser);
$adaemailnyauser = mysql_num_rows($resultemailnyauser);
$rowemailnyauser = mysql_fetch_array($resultemailnyauser);
$userlogin = $rowemailnyauser[0];	
	   
//$userlogin = anti_injection($_POST['userlogin']); 
//$passlogin=md5($_POST['passlogin']);

$cekvalid = mysql_query("SELECT * FROM validation WHERE username='$userlogin'");
$ckvalid = mysql_num_rows($cekvalid);
if($mailconfirm == 1 && $ckvalid){
header("location: ./login.php?result=noactive_mail&user=".$userlogin."".$useref."");
exit;
} else {

$usl=base64_encode($userlogin);	
$psy=base64_encode($passlogin);	
	$_SESSION["user_session"] = $userlogin;
	setcookie("user", $userlogin, strtotime( '+1 days' ), "/", "", "", TRUE);
...
$sqlus="SELECT username FROM member WHERE username = '".$userlogin."'"; 
...
$db->select("username, pass, nama, sponsor, email, status, blokir, logmember, hp", "member", "username='".$userlogin."' and pass='$passlogin'");
if (!$db->num_rows()){
...
if($googleauntentic == 1 && $authgoogles == 1){
header("location: ./verification.php?u=".$usl."&p=".$psy."");
exit;
} else {			  
